...
var mssqlconfigCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		metaObj.Package = genPackageName

		fileAbsPath := output + "/gen_mssql_config.go"
		executeTpl(fileAbsPath, "mssql_config", metaObj)
	},
}

func init() {
	mssqlconfigCmd.PersistentFlags().StringVarP(&output, "output", "o", "", "output path")
	mssqlconfigCmd.PersistentFlags().StringVarP(&genPackageName, "package", "p", "", "package name")

	RootCmd.AddCommand(mssqlconfigCmd)
